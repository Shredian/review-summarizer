...
class ProductUpdate(BaseModel):
    name: str | None = Field(None, max_length=255, description="Название продукта")
    description: str | None = Field(None, description="Описание продукта")
...
async def list_products(
    ...
    return [
        ProductWithReviewsCount(product=p, reviews_count=count) for p, count in products_with_counts
    ]
...
async def get_product(
    product_id: Annotated[UUID, Path(description="UUID продукта")],
) -> Product:
    app = Container.product_application()
    try:

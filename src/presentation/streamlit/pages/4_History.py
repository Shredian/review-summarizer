...
try:
    ...
    if selected_summary:
        ...
        with col2:
            st.markdown(
                f"**Метод:** {selected_summary.method} ({selected_summary.method_version or 'N/A'})"
            )
        with col3:
        ...
        with st.expander("📊 Статистика отзывов"):
            ...
            with stat_cols[1]:
                st.metric(
                    "Средний рейтинг",
                    f"{selected_summary.rating_avg:.1f}" if selected_summary.rating_avg else "-",
                )
            with stat_cols[2]:
                st.metric(
                    "Дата (мин)",
                    selected_summary.date_min.strftime("%Y-%m-%d")
                    if selected_summary.date_min
                    else "-",
                )
            with stat_cols[3]:
                st.metric(
                    "Дата (макс)",
                    selected_summary.date_max.strftime("%Y-%m-%d")
                    if selected_summary.date_max
                    else "-",
                )
        ...
        if selected_summary.has_key_phrases():
            ...
            positive_phrases = [
                kp for kp in selected_summary.key_phrases if kp.sentiment == "positive"
            ]
            negative_phrases = [
                kp for kp in selected_summary.key_phrases if kp.sentiment == "negative"
            ]
            neutral_phrases = [
                kp for kp in selected_summary.key_phrases if kp.sentiment == "neutral"
            ]

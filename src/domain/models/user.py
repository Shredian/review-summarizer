...
class User(BaseModel):
    id: UUID | None = Field(default=None, description="Уникальный идентификатор пользователя")
    display_name: str = Field(..., max_length=255, description="Отображаемое имя пользователя")
    profile_url: str | None = Field(default=None, description="URL профиля пользователя")
    created_at: datetime = Field(default_factory=datetime.now, description="Дата создания записи")

    def update(self, display_name: str | None = None, profile_url: str | None = None) -> None:

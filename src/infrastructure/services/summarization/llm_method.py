...
class LLMSummarizationMethod(BaseSummarizationMethod):
    ...
    async def summarize(
        ...
        logger.info(
            f"Параметры: category={category}, use_soft_mode={use_soft_mode}, reviews_count={len(reviews)}"
        )
    ...
    def _extract_frequent_phrases(self, processed_reviews: list[str], top_n: int = 80) -> list[str]:
    ...
    async def _filter_relevant_phrases(self, phrases: list[str], category: str) -> list[PhraseData]:
    ...
    async def _generalize_phrases(
        ...
        prompt = prompt_template.format(category=category, phrases=formatted_phrases_str)
    ...
    async def _generate_reviews(
        ...
        positive_phrases = [
            p.generalized_phrase or p.phrase for p in phrases if p.sentiment == "positive"
        ]
        negative_phrases = [
            p.generalized_phrase or p.phrase for p in phrases if p.sentiment == "negative"
        ]

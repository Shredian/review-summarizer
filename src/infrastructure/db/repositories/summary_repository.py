...
class SummaryRepository:
    """Сохранённые результаты суммаризации."""
    ...
    async def get(self, summary_id: UUID) -> Summary:
        async with self.session_factory() as session:
            result = await session.execute(select(SummaryDB).where(SummaryDB.id == summary_id))
    ...
    async def list(self, limit: int = 100, offset: int = 0) -> list[Summary]:
        async with self.session_factory() as session:
            result = await session.execute(
                select(SummaryDB).order_by(SummaryDB.created_at.desc()).limit(limit).offset(offset)
            )
